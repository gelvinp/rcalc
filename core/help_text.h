...
R"foo(Most calculators use a style of notation similar to '1 + 2 = 3', known as _infix_ notation.
Reverse Polish Notation, or RPN for short, is instead a _postfix_ notation, where operators are placed after their operands (i.e. '1 2 + = 3').
Intuitively, operands are placed into a _stack_, and then operators operate off the top of the stack.

To understand how this works in RCalc, let's examine the `add` operator. Add takes two arguments, referred to as arg0 and arg1, and returns their sum.
If you type 1 <enter> 2 <enter>, the stack will have two values. In this example, 1 will become arg0 and 2 will become arg1. Applying the add operator to this stack gives us `1 + 2`.
...
Notice how we enter our operands first, from the outermost inward, and then apply our operators second, from the innermost outward.
People who become proficient at using RPN can enter calculations with great speed.
)foo"
    },
    {
        "Entering Values",
...
Vectors can be _swizzled_, allowing you to access and rearrange components in a vector. When a vector is at the top of the stack (bottom of the screen), enter a period ('.') followed by 1-4 of either xyzw or rgba to select the first, second, third, or fourth component of the vector.
Below are some examples of swizzled vectors:
[1, 2].yx -> [2, 1]
[1, 2, 3].xzy -> [1, 3, 2]
