...
class UnitsMapBuilder:
    ...
    def _build_std_map(self):
        lines = [
            ...
            'namespace RCalc {',
            '',
            'std::map<std::string, Unit const * const, std::less<>> units_map;',
            '',
            'void UnitsMap::build() {',
            '\tif (built) { return; }',
        ]
        ...
        lines.extend([
            ...
            '}',
            '',
            'std::optional<const Unit*> UnitsMap::find_unit(std::string_view str) {',
            '\tif (!built) { build(); }',
            '\tstd::map<std::string, Unit const * const, std::less<>>::iterator it = units_map.find(str);',
            '\tif (it != units_map.end()) {',
            '\t\treturn it->second;',
            '\t}',
    ...
    def _build_gperf_map(self, gperf_path: str):
        ...
        lines.extend([
            ...
            'std::optional<const Unit*> UnitsMap::find_unit(std::string_view str) {',
            '\tif (!built) { build(); }',
            '\tif (Units::GPerf::in_word_set(str.data(), str.size()) != nullptr) {',
            '\t\treturn units_map[Units::GPerf::hash(str.data(), str.size()) - MIN_HASH_VALUE];',
            '\t}',
            '\telse { return std::nullopt; }',
            '}',
            '',
