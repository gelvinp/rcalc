...
class Command:
    ...
    def build(self, scope_name):
        ...
        if len(self.aliases) > 0:
            lines.extend([
                f'constexpr const char * CMDALIASES_{scope_name}_{self.name}[] {{',
                ',\n'.join([f'\t"{alias}"' for alias in self.aliases]),
                '};',
                '',
            ])

            alias_name = f'CMDALIASES_{scope_name}_{self.name}'

        lines.extend([
            f'constexpr CommandMeta CMDMETA_{scope_name}_{self.name} {{',
            f'\t"{self.name}",',
            f'\t"{self.description}",',
            f'\t{alias_name}',
            '};',
...
class Scope:
    ...
    def build(self):
        ...
        lines.append(f'constexpr CommandMeta const * SCOPECMDS_{self.name}[] {{')
        ...
        lines.extend([
            '};',
            '',
            f'constexpr ScopeMeta SCOPEMETA_{self.name} {{',
            f'\t"{self.name}",',
    ...
    def _build_std_map(self, scopes: list[str]):
        ...
        for scope_name in scopes:
            lines.extend([
                f'/* Scope {scope_name} */',
                '',
                f'std::map<std::string, Command<{scope_name}>, std::less<>> CMDMAP_{scope_name};',
                '',
                'template<>',
            ...
            lines.extend([
                ...
                '',
                'template<>',
                f'bool CommandMap<{scope_name}>::has_command(std::string_view str) {{',
                '\tif (!built) { build(); }',
                f'\treturn CMDMAP_{scope_name}.contains(str);',
                '}',
                '',
                'template<>',
                f'void CommandMap<{scope_name}>::execute(std::string_view str, {scope_name}& scope) {{',
                '\tif (!built) { build(); }',
                f'\tCommand<{scope_name}> cmd = CMDMAP_{scope_name}.find(str)->second;',
                f'\t(scope.*cmd)();',
                '}',
                '',
                'template<>',
    ...
    def _build_gperf_map(self, scopes: list[str], gperf_path: str):
        ...
        for scope_name in scopes:
            ...
            lines.extend([
                ...
                f'bool CommandMap<{scope_name}>::has_command(std::string_view str) {{',
                '\tif (!built) { build(); }',
                f'\treturn Commands::GPerf::SCOPELOOKUP_{scope_name}(str.data(), str.size()) != nullptr;',
                '}',
                '',
                'template<>',
                f'void CommandMap<{scope_name}>::execute(std::string_view str, {scope_name}& scope) {{',
                '\tif (!built) { build(); }',
                f'\t(scope.*CMDMAP_{scope_name}[Commands::GPerf::SCOPEHASH_{scope_name}(str.data(), str.size()) - MIN_HASH_VALUE])();',
                '}',
                '',
                'template<>',
                f'CommandMap<{scope_name}>& CommandMap<{scope_name}>::get_command_map() {{',
