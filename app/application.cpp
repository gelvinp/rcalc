...
void Application::run(Platform& platform, AppConfig config) {
  if (config.quiet) {
  ...
  Application app{};
  app.stack.emplace_back("Welcome to RCalc!", std::nullopt, std::nullopt, false, false);

  app._run(platform);
}


void Application::_run(Platform& platform) {
    while(!platform.close_requested) {
...
Application::StackItem::StackItem(std::string input, std::optional<std::string> output, std::optional<double> result, bool is_error, bool calculate_now) :
    input(input), output(output), is_error(is_error), result(result) {
  if (calculate_now)
    recalculate_size();
}
